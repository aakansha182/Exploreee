...
app.post('/upload-image', upload.single('image'), async (req, res) => {
  if (!req.file) {
    return res.status(400).send({ status: 'error', message: 'Please upload an image.' });
  }
  ...
  try {
    ...
    res.status(500).json({ status: 'error', message: 'Error saving image in database' });
  }
});
...
app.get('/get-image', async (req, res) => {
  try {
    ...
    res.status(500).json({ status: 'error', message: 'Error fetching images from database' });
  }
});
app.listen(3001, () => {
